...
public class Graph {
	 ...
	 * Knotenobjekt nicht im Graphen enthalten, wird null zurueckgeliefert.
	 */
	 ...
	 * keinen Knoten mit demselben ID-Eintrag wie pVertex im Graphen gibt und
	 * pVertex eine ID ungleich null hat. Ansonsten passiert nichts.
	 */
	public void addVertex(Vertex pVertex) {
		...
		if (pVertex != null && pVertex.getID() != null) {
			...
			// Wenn die ID noch frei ist, den Vertex einfuegen, sonst nichts
			// tun.
			if (freeID) {
	...
	/**
	 * Der Auftrag fuegt die Kante pEdge in den Graphen ein, sofern beide durch
	 * die Kante verbundenen Knoten im Graphen enthalten sind, nicht identisch
	 * sind und noch keine Kante zwischen den Knoten existiert. Ansonsten
	 * passiert nichts.
	 */
	public void addEdge(Edge pEdge) {
		...
		if (pEdge != null) {
			...
			if (vertexPair[0] != null && vertexPair[1] != null && this.getVertex(vertexPair[0].getID()) == vertexPair[0]
					&& this.getVertex(vertexPair[1].getID()) == vertexPair[1]
					&& this.getEdge(vertexPair[0], vertexPair[1]) == null && vertexPair[0] != vertexPair[1]) {
				// Kante einfuegen.
				edges.append(pEdge);
			}
		}
	 ...
	 * Kanten, die mit ihm inzident sind. Ist der Knoten pVertex nicht im
	 * Graphen enthalten, passiert nichts.
	 */
	...
	/**
	 * Die Anfrage liefert alle Nachbarn des Knotens pVertex als neue Liste vom
	 * Typ List<Vertex>. Hat der Knoten pVertex keine Nachbarn in diesem Graphen
	 * oder ist gar nicht in diesem Graphen enthalten, so wird eine leere Liste
	 * zurueckgeliefert.
	 */
	public List<Vertex> getNeighbours(Vertex pVertex) {
		...
		while (edges.hasAccess()) {
			...
			// Wenn ein Knoten der Kante pVertex ist, den anderen als Nachbarn
			// in die Ergebnisliste einfuegen.
	 ...
	 * pVertex. Hat der Knoten pVertex keine inzidenten Kanten in diesem Graphen
	 * oder ist gar nicht in diesem Graphen enthalten, so wird eine leere Liste
	 * zurueckgeliefert.
	 */
	public List<Edge> getEdges(Vertex pVertex) {
		...
		while (edges.hasAccess()) {
			...
			// Wenn ein Knoten der Kante pVertex ist, dann Kante als inzidente
			// Kante in die Ergebnisliste einfuegen.
			Vertex[] vertexPair = edges.getContent().getVertices();
			if (vertexPair[0] == pVertex) {
				result.append(edges.getContent());
			} else {
	...
	/**
	 * Die Anfrage liefert die Kante, welche die Knoten pVertex und
	 * pAnotherVertex verbindet, als Objekt vom Typ Edge. Ist der Knoten pVertex
	 * oder der Knoten pAnotherVertex nicht im Graphen enthalten oder gibt es
	 * keine Kante, die beide Knoten verbindet, so wird null zurueckgeliefert.
	 */
